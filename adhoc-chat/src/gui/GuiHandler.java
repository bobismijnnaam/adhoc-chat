...
public class GuiHandler implements java.awt.event.ActionListener, AdhocListener{
	...
	private void processMessage(String group) {
		...
		if (!message.equals("") && message.trim().length() > 0 ) {
			...
			newMessage = mainScreen.addMessage(message, mainScreen.getUsername(), "#f22d2d", "#d10c0c", false, group, date);
			frame.pack();
	...
	/**
	 * Triggered when a new connection is esthablished, adds a new chat and user to list of neccesary
	 */
	@Override
	public void newConnection(Connection connection) {
		...
		// picks a color from the gradientcolor list and associates it with the username
	...
	public void removedConnection(Connection connection) {
		...
		// because bob wants to reread messages from his lovers mainScreen.changeChat("GroupChat");
	}
